...
static marketplace and schema. The validation workflow also hands that generated
index and every maintained extension directory to the current 5gpn `beta` Go
tests, so a marketplace or native-manifest contract drift fails before delivery.
