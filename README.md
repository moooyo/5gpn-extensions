...
The index is published in two profiles from that one build. `v1` is frozen at
what the stable core accepts: it parses the index with `DisallowUnknownFields`,
so a field added there is not additive — a core that does not know it refuses
the whole document and loses its extension catalogue. `v1beta` carries the
typed policy projection for cores that have learned to read it. `--profile` is
required rather than defaulted, because the default would silently decide which
bytes reach every deployed gateway.

The validation workflow hands each profile to the core that consumes it: `v1`
to 5gpn `main`, `v1beta` to 5gpn `beta`. Checking only against `beta` would be
blind to the one failure this gate exists to prevent, since a field only `beta`
knows passes there and breaks every stable gateway.
